...
pct exec $container_id -- bash -c "sshpass -p $extra_admin_pw sudo -u $replication_account ssh-copy-id -o StrictHostKeyChecking=no root@10.0.254.251"


#msg_info "generate ssh Key Pihole1 and copy to pihole2"
pct exec $container_id -- bash -c "sudo -u $replication_account ssh-keygen -t rsa -b 4096 -f /home/$replication_account/.ssh/id_rsa -q -N ''"
pct exec $container_id -- bash -c "sudo -u $replication_account ssh-copy-id -o StrictHostKeyChecking=no root@$container_ip"
#msg_ok "generated ssh Key Pihole1 and copied  to Pihole2"

#msg_info "generate ssh Key or Pihole2 and copy to Pihole1"
pct exec $container_id2 -- bash -c "sudo -u $replication_account ssh-keygen -t rsa -b 4096 -f /home/$replication_account/.ssh/id_rsa -q -N ''"
pct exec $container_id2 -- bash -c "sudo -u $replication_account ssh-copy-id -o StrictHostKeyChecking=no root@$container_ip2"
